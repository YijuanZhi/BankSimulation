...
int main (int argc, char *argv[]) {
  ...
  for(int i =0; i < atm_count-1; i++){  //create the ATM processes
    int to_atmfd[2];
    pipe(to_atmfd); //create pipe between read end and write end
    atm_out_fd[i] = to_atmfd[1];
    int to_bankfd[2];
    pipe(to_bankfd); //create pipe between read end and write end
    bank_in_fd[i] = to_bankfd[0];
    if(fork()== 0){ //child process
      close(to_atmfd[1]); //close on the ATM’s output pipe file descriptor
      close(to_bankfd[0]); //close on the bank’s input pipe file descriptor
