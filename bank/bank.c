...
int bank (int atm_out_fd[], Command *cmd, int *atms_remaining) {
  ...
  if(check_valid_atm(i)!=SUCCESS){ //check that the ATM ID unpacked from the received command is a valid ATM.
    return ERR_UNKNOWN_ATM;
  }
  //CONNECT
   if(c == CONNECT){
    MSG_OK(&bankcmd,0,f,t,a); //the bank receives this message
  ...
  if(c==DEPOSIT){
    if(check_valid_account(t)==SUCCESS){
      ...
      MSG_ACCUNKN(&bankcmd,0,t);// it is not a valid account
      result = checked_write(atm_out_fd[i],&bankcmd,MESSAGE_SIZE);
    }
  }
  //WITHDRAW
  ...
  if(check_valid_account(f)==SUCCESS){
   ...
   MSG_NOFUNDS(&bankcmd,0,f,a);//insufficient funds
  result = checked_write(atm_out_fd[i],&bankcmd,MESSAGE_SIZE);
  }
}
  else{
...
if(c==TRANSFER){
  if(check_valid_account(f)!=SUCCESS){
    MSG_ACCUNKN(&bankcmd,0,f); //unknown accounts
    result = checked_write(atm_out_fd[i],&bankcmd,MESSAGE_SIZE);
  }
  else if (check_valid_account(t)!=SUCCESS){
    MSG_ACCUNKN(&bankcmd,0,t); //unknown accounts
    result = checked_write(atm_out_fd[i],&bankcmd,MESSAGE_SIZE);
  }
  else {
     if(accounts[f] < a){
