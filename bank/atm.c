...
int atm (int bank_out_fd, int atm_in_fd, int atm_id, Command *cmd) {
  ...
  if(atm_id == i){ //make sure that the ID in the unpacked command is the same as this ATM’s ID
     if(c==CONNECT || c==EXIT || c ==DEPOSIT ||c==TRANSFER || c==BALANCE || c== WITHDRAW){
      status = checked_write(bank_out_fd, cmd, MESSAGE_SIZE); //send the cmd received from the trace file to the bank process
       if(status != SUCCESS){
        return status;
      }
      status = checked_read(atm_in_fd, &atmcmd, MESSAGE_SIZE);// read the response from the bank using the checked_read function, into the atmcmd command buffer
      if(status != SUCCESS){
          return status;
        }
        cmd_unpack(&atmcmd,&c, &i, &f, &t, &a); //unpack the command into its individual parts
            if(c== OK){
              status = SUCCESS;
            }
            if(c == NOFUNDS){
